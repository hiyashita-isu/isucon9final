...
func trainSearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	minStationID := toStation.ID
	maxStationID := fromStation.ID
	if minStationID > maxStationID {
		minStationID = fromStation.ID
	...
	if trainClass == "" {
		...
		inQuery, inArgs, err = sqlx.In(query, date.Format("2006/01/02"), trainClass, isNobori, minStationID, minStationID, maxStationID, maxStationID)
	}
	if err != nil {
		log.Print(err)
		errorResponse(w, http.StatusBadRequest, err.Error())
...
FROM reservations r, seat_reservations s
WHERE
